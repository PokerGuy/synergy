...
module.exports.deploy = async (event, context) => {
    ...
    if (!(setupComplete)) {
        ...
        try {
            const setupActivities = await Promise.all([waitForGit, waitForCLI, waitForIOTGateway]);
            console.log("Lambda is ready with git, awscli, and iotgateway...");
            if (!token) {
                const kms = new AWS.KMS({region: "us-west-2"});
                const data = await kms.decrypt({CiphertextBlob: Buffer(process.env.GIT_TOKEN, "base64")}).promise();
                token = data.Plaintext.toString("ascii");
                await createLock(event);
                const exitCode = await runScript(event);
                return await finishBuild(event, exitCode);
            } else {
                await createLock(event);
                const exitCode =  await runScript(event);
                return await finishBuild(event, exitCode);
...
const cliSetup = () => {
    return new Promise((fullfill, reject) => {
        ...
        reader.on("end", () => {
            console.log("AWS CLI untarred and ready");
            fullfill();
        });
    })
};
...
const finishBuild = async (event, exit) => {
    const msg = JSON.parse(event.Records[0].Sns.Message);
    const buildTime = msg.buildTime;
    ...
    const p = {
        ...
        Item: {
            ...
            output: `Exited with code ${exit.toString()}`,
            type: "end"
    ...
    try {
        const promises = await Promise.all([Step1, Step2, Step3]);
    } catch (e) {
        console.log("Error:");
        console.log(e);
    }
...
const runScript = (event) => {
    return new Promise((fulfill, reject) => {
        ...
        cloneScript.on("exit", async (code) => {
            console.log(`Shell script has stopped with code ${code}`);
            fulfill(code);
        });
    })
};
